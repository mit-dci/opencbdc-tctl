...
func (t *TestRunManager) PreseedShards(
	...
	} else if tr.Architecture == "default-inmem" {
		targetPath = "shard_preseed.tar"
		filePrefix = "atom_inmem_shard_preseed_"
		tarCreateNoDir = true
	}
	...
	preseedShard := func(r *common.TestRunRole, trn *common.TestRun, shardStart, shardEnd int, utxoCount int64, envID []byte) {
		...
		// 2PC in memory shard uses a single file, not a folder - have to rename
		// the file coming from the tar archive
		if trn.Architecture == "2pc-inmem" ||
			trn.Architecture == "default-inmem" {

			prefix := "atom_inmem_shard_preseed_"
			if trn.Architecture == "2pc-inmem" {
				prefix = "2pc_shard_preseed_"
			}
			...
			if trn.Architecture == "default-inmem" {
				inmemPreseedTarget = t.SubstituteParameters(
					[]string{"shard_preseed_%IDX%"},
					r,
					trn,
				)
			}
