...
func NewAwsManager() *AwsManager {
	...
	go func() {
		...
		for _, lt := range am.launchTemplates {
			if lt.Region == "us-east-1" && lt.VCPUCount == 2 {
				smallID = lt.TemplateID
			} else if lt.Region == "us-east-1" && lt.VCPUCount == 8 {
				largeID = lt.TemplateID
			}
		}
		common.ConfigureLaunchTemplatesForDefaultTests(smallID, largeID)
	}()
